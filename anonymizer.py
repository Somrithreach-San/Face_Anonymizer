...
def process_video(
    ...
    writer = None
    # Try MP4: H.264 first, then mp4v
    for code in ("avc1", "H264", "mp4v"):
        try:
            fourcc = cv2.VideoWriter_fourcc(*code)
            w = cv2.VideoWriter(str(out_p), fourcc, fps, (width, height))
            if w.isOpened():
                writer = w
                break
            else:
            ...
            continue
    # Fallback: WebM VP9/VP8 by switching extension if MP4 failed
    if writer is None:
        out_webm = out_p.with_suffix(".webm")
        for code in ("VP90", "VP80"):
            try:
                fourcc = cv2.VideoWriter_fourcc(*code)
                w = cv2.VideoWriter(str(out_webm), fourcc, fps, (width, height))
                if w.isOpened():
                    out_p = out_webm
                    writer = w
                    break
    ...
    if writer is None or not writer.isOpened():
        logger.error("Cannot open output writer with common codecs for: %s", output_path)
        return "", 0
